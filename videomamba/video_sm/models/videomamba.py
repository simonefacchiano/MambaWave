...
class VisionMamba(nn.Module):
    ...
    def forward_features(self, x, inference_params=None):
        # Esempio con un video casuale: Prima del patch embed: torch.Size([1, 3, 8, 224, 224])
        x = self.patch_embed(x)
         # Esempio con un video casuale: Dopo del patch embed: torch.Size([1, 384, 8, 14, 14]) --> ha dimezzato il numero di parametri
        B, C, T, H, W = x.shape
        ...
        for idx, layer in enumerate(self.layers):
            if self.use_checkpoint and idx < self.checkpoint_num:
                ...
                # qui, sia hidden_states che residual hanno la stessa dimensione [..., ..., 384] (nel caso di videomamba_small)
            else:
                hidden_states, residual = layer(
                    hidden_states, residual, inference_params=inference_params
                )
    ...
    def forward(self, x, inference_params=None):
        ...
        #x = self.head(self.head_drop(x)) #@simone
        return x
...
def videomamba_small(pretrained=False, **kwargs):
    '''This is the one we used for the AutoEncoder @simone'''
    model = VisionMamba(
        patch_size=16, 
        embed_dim=384, 
        depth=24, 
...
@register_model
def videomamba_big(pretrained=False, **kwargs):
    '''Made by @simone '''
    model = VisionMamba(
        patch_size=16, 
        embed_dim=784, 
    ...
    if pretrained:
        print('load pretrained weights')
        state_dict = torch.load(_MODELS["videomamba_b16_in1k"], map_location='cpu')
        load_state_dict(model, state_dict, center=True)
    return model
